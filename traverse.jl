...
function _norm_layer_forward(
  ...
  ) where {T, N} # trainmode or testmode without tracked stats
...
pt_path="yolov7_training.pt"
pt = Pickle.Torch.THload(pt_path)

model = pt["model"].args[2]["_modules"]["model"].args[2]["_modules"]

model_flux = OrderedCollections.OrderedDict()
...
for (k, m) in model
    ...
    elseif m.args[1].args[1].head == Symbol("torch.nn.modules.upsampling.Upsample")
        ...
        up = Flux.Upsample(scale_factor, mode)
        model_flux[k] = up
